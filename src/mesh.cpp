...
void Mesh::draw() {
    glBindVertexArray(VAO);
    glDrawElements(static_cast<GLenum>(mode), triangles.size(), GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
}

void Mesh::DrawInstanced(int instanceCount) {
    glBindVertexArray(VAO);
    glDrawElementsInstanced(static_cast<GLenum>(mode), triangles.size(), GL_UNSIGNED_INT, (GLvoid *) 0, instanceCount);
    glBindVertexArray(0);
}
